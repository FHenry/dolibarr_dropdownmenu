    ...
    if ($num > 0) {
        for ($i = 0; $i < $num; $i++) {
            if ($arrayMenu[$i]['enabled']) {
				...
				print_text_menu_entry($arrayMenu[$i]['titre'], $arrayMenu[$i]['enabled'], (($arrayMenu[$i]['url'] != '#' && !preg_match('/^(http:\/\/|https:\/\/)/i', $arrayMenu[$i]['url'])) ? DOL_URL_ROOT:'').$arrayMenu[$i]['url'], $arrayMenu[$i]['id'], $arrayMenu[$i]['idsel'], $arrayMenu[$i]['classname'], ($arrayMenu[$i]['target'] ? $arrayMenu[$i]['target'] : $atarget), $arrayMenu[$i]);
        ...
        if ($resql) {
            ...
            if ($numr > 0) {
                while ($i < $numr) {
                    ...
                    if ($objp->nature == 3 && (isModEnabled('fournisseur') && empty($conf->global->MAIN_USE_NEW_SUPPLIERMOD) || !empty($conf->supplier_invoice->enabled))) $nature = "purchases";
